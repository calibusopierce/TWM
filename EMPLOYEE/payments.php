...
// The payments page itself is only meaningful for an Approved loan — a
// Proposal has no payments to make yet, and Fully Paid/Cancelled are closed.
...
if (isset($_POST['ajax_action'])) {
    ...
    if ($_POST['ajax_action'] === 'record_payment') {
        // Payments may only be recorded against an Approved loan.
        $status_chk = sqlsrv_query($conn, "SELECT Status FROM TBL_Loan WHERE LoanID = ?", [$loan_id]);
        $status_row = $status_chk ? sqlsrv_fetch_array($status_chk, SQLSRV_FETCH_ASSOC) : null;
        if (!$status_row || trim($status_row['Status'] ?? '') !== 'Approved') {
            echo json_encode(['ok' => false, 'error' => 'Payments can only be recorded on an Approved loan.']);
            exit;
        }
        ...
        $pay_id = $row['LoanPaymentID'];

        // Link to statement row
        sqlsrv_query($conn, "UPDATE TBL_Loan_Statement SET PaymentID = ? WHERE StatementID = ?",
            [$pay_id, $stmt_id]);

        // Update loan paid/balance totals
        sqlsrv_query($conn, "
            UPDATE TBL_Loan SET
            ...
            WHERE LoanID = ?",
            [$pay_amt, $pay_amt, $user, $loan_id]);

        // Auto-flip to Fully Paid if balance <= 0
        sqlsrv_query($conn, "
            UPDATE TBL_Loan SET Status = 'Fully Paid'
            WHERE LoanID = ? AND BalanceAmount <= 0 AND Status = 'Approved'",
            [$loan_id]);
    ...
    if ($_POST['ajax_action'] === 'unlink_payment') {
        ...
        $pr   = sqlsrv_query($conn, "SELECT PaymentAmount FROM TBL_Loan_Payment WHERE LoanPaymentID = ?", [$pay_id]);
        $prow = $pr ? sqlsrv_fetch_array($pr, SQLSRV_FETCH_ASSOC) : null;
        $amt  = $prow ? (float)$prow['PaymentAmount'] : 0;

        sqlsrv_query($conn, "UPDATE TBL_Loan_Statement SET PaymentID = NULL WHERE StatementID = ?", [$stmt_id]);
        sqlsrv_query($conn, "DELETE FROM TBL_Loan_Payment WHERE LoanPaymentID = ?", [$pay_id]);
        ...
        if ($amt > 0) {
            sqlsrv_query($conn, "
                UPDATE TBL_Loan SET
                    ...
                    UserUpdate    = ?, UpdateDateTime = GETDATE()
                WHERE LoanID = ?",
                [$amt, $amt, $user, $loan_id]);
        }

        // Reverse an auto Fully Paid flip if the loan is no longer fully covered.
        sqlsrv_query($conn, "
            UPDATE TBL_Loan SET Status = 'Approved'
            WHERE LoanID = ? AND BalanceAmount > 0 AND Status = 'Fully Paid'",
            [$loan_id]);
...
<head>
  ...
  <title>Payments · <?= htmlspecialchars($loan['ReferenceNumber'] ?? 'Loan') ?> · Tradewell</title>
...
<div class="main-wrapper">
  ...
  <div class="info-grid3">
    <div class="info-card">
      <div class="info-card-header">Loan Amount</div>
      <div class="info-card-body">₱ <?= number_format($loan_amt, 2) ?></div>
    </div>
    <div class="info-card">
        ...
        <tbody>
          <?php if (empty($schedule)): ?>
            <tr><td colspan="5" style="text-align:center;padding:2rem;color:var(--text-muted);">
              No schedule rows for this loan yet.
            </td></tr>
          <?php else: ?>
            ...
            <tr class="<?= $paid ? 'paid-row' : '' ?>">
              ...
              <td><?= htmlspecialchars($month_v) ?></td>
...
function submitPayment() {
  ...
  fetch(`payments.php?id=${loanId}`, { method: 'POST', body: fd })
    .then(r => r.json())
    .then(data => {
...
function unlinkPayment(stmtId, payId) {
  if (!confirm('Remove this payment? The amount will be reversed from the loan balance.')) return;
  const fd = new FormData();
