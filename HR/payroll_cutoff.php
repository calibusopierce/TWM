...
$deptList = [];
if ($canFilterDept) {
    $dStmt = sqlsrv_query($conn, "SELECT DISTINCT RTRIM(Department) AS Department FROM View_Payroll_Day_Late WHERE Department IS NOT NULL AND Department <> '' ORDER BY Department");
    if ($dStmt) { while ($r = sqlsrv_fetch_array($dStmt, SQLSRV_FETCH_ASSOC)) { $deptList[] = $r['Department']; } sqlsrv_free_stmt($dStmt); }
}

$categoryList = [];
$catStmt = sqlsrv_query($conn, "SELECT DISTINCT RTRIM(Category) AS V FROM View_Payroll_Day_Late WHERE Category IS NOT NULL AND Category <> '' " . dc($filterDeptSafe) . " ORDER BY V");
if ($catStmt) { while ($r = sqlsrv_fetch_array($catStmt, SQLSRV_FETCH_ASSOC)) { $categoryList[] = $r['V']; } sqlsrv_free_stmt($catStmt); }

$pGroupList = [];
$pgStmt = sqlsrv_query($conn, "SELECT DISTINCT RTRIM(PayrollGroup) AS V FROM View_Payroll_Day_Late WHERE PayrollGroup IS NOT NULL AND PayrollGroup <> '' " . dc($filterDeptSafe) . " ORDER BY V");
if ($pgStmt) { while ($r = sqlsrv_fetch_array($pgStmt, SQLSRV_FETCH_ASSOC)) { $pGroupList[] = $r['V']; } sqlsrv_free_stmt($pgStmt); }

$yearList = [];
$yStmt = sqlsrv_query($conn, "SELECT DISTINCT PayrollYear AS V FROM View_Payroll_Day_Late WHERE PayrollYear IS NOT NULL " . dc($filterDeptSafe) . " ORDER BY V DESC");
if ($yStmt) { while ($r = sqlsrv_fetch_array($yStmt, SQLSRV_FETCH_ASSOC)) { $yearList[] = $r['V']; } sqlsrv_free_stmt($yStmt); }

$monthNames = [1=>'January',2=>'February',3=>'March',4=>'April',5=>'May',6=>'June',7=>'July',8=>'August',9=>'September',10=>'October',11=>'November',12=>'December'];

// ── Main query — pull every Day1..Day31 column + TotalLate ──
...
if ($stmt === false) { $debugLog['payroll_cutoff'] = sqlsrv_errors(); }
...
<title>Payroll Cutoff — HR · TWM</title>
...
.co-total-late { font-family:'JetBrains Mono',monospace; font-weight:700; }
.co-total-late.zero { color:#94a3b8; font-weight:400; }
.co-total-late.warn { color:#ca8a04; }
.co-total-late.danger { color:#dc2626; }
...
.co-table td.co-day.late { color:#dc2626; font-weight:700; }
...
.co-legend-swatch.late { background:#fecaca; }
...
<div class="hr-page-header">
  <div>
    <div class="hr-page-title">🧮 <span style="color:#2563eb;">Payroll Cutoff</span> Late Summary</div>
...
<div class="hr-table-card">
  ...
  <div class="co-legend">
    <span class="co-legend-item"><span class="co-legend-swatch late"></span> Minutes late</span>
  ...
  <div class="co-tab-panel<?= $tabId === 'co1' ? ' active' : '' ?>" id="coPanel-<?= $tabId ?>">
    <div class="hr-table-toolbar">
      <div class="hr-table-title">
        🧮 <?= htmlspecialchars($tab['label']) ?> Late Summary
      ...
      <div class="hr-table-actions">
        ...
        <button class="hr-btn hr-btn-ghost" onclick="exportExcel('<?= $tabId ?>', 'payroll_<?= $tabId ?>_late_summary')">
          <i class="bi bi-file-earmark-excel"></i> Excel
        </button>
        <button class="hr-btn hr-btn-ghost" onclick="printTable('<?= $tabId ?>', '<?= htmlspecialchars($tab['label']) ?> Late Summary')">
...
// One <td> per day column: Sundays always show "DAY OFF". Numeric values
// render as late-minutes; non-numeric codes (A = Absent, HD = Half Day, or
// any other text code the view returns) render as-is instead of being
// coerced to 0.
function rowCo(r, id) {
    ...
    const dayCells = COLS[id].map(c => {
        ...
        if (/^-?\d+(\.\d+)?\s*L?$/i.test(str)) {
            ...
            return `<td class="co-day ${mins > 0 ? 'late' : 'zero'}">${mins}</td>`;
        }
        ...
        if (code === 'L')  return `<td class="co-day zero">—</td>`; // bare "L" with no number — not a real code
    ...
    return `<tr>
      ...
      ${dayCells}
      <td><span class="co-total-late ${cls}">${cls==='zero' ? '—' : total + ' min'}</span></td>
    </tr>`;
}
...
function printTable(id, title) {
    ...
    win.document.write(`<!DOCTYPE html><html><head>
      ...
      <style>
        ...
        td.co-day.late { color:#dc2626 !important; font-weight:700; }
        ...
        .co-total-late { font-weight:700; }
        .co-total-late.zero   { color:#94a3b8; font-weight:400; }
        .co-total-late.warn   { color:#ca8a04 !important; }
        .co-total-late.danger { color:#dc2626 !important; }
