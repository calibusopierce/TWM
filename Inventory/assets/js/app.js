...
document.addEventListener('DOMContentLoaded', function () {
  ...
  openBtns.forEach(function (btn) {
    btn.addEventListener('click', function () {
      // [data-open-modal] with a value targets that specific overlay by id
      // (needed on pages with more than one "create" modal, e.g. Requests'
      // Item Request + Repair Request buttons). [data-open-modal] with no
      // value keeps the old behavior: open the page's one primary
      // create/edit modal (overlays[0]). Other modals (like a confirmation
      // dialog) are opened directly by their own script either way.
      const targetId = btn.getAttribute('data-open-modal');
      const overlay = targetId ? document.getElementById(targetId) : overlays[0];
      if (!overlay) return;
  ...
  /* ---- Client-side search filter: any .searchbox input filters its own .card's table rows ----
     Skips inputs marked [data-custom-search] -- pages with more than one
     table in the same .card (e.g. Requests' Item/Repair tabs) wire up
     their own per-table filtering instead, since this generic version
     would filter every table in the card off of one search box. */
  document.querySelectorAll('.searchbox input:not([data-custom-search])').forEach(function (input) {
