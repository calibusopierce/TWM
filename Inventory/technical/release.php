...
// that still has stock left. Availability is computed differently
// depending on TrackingMethod:
//   Individual Unit Tracking -> unit barcodes not currently checked out
//   Quantity-Based           -> received minus released (net of returns)
$stockSql = "
SELECT * FROM (
    SELECT
        pi.POItemID,
        ...
        po.PONumber,
        CASE WHEN COALESCE(pi.TrackingMethod, 'Quantity-Based') = 'Individual Unit Tracking' THEN
            (SELECT COUNT(*) FROM TBL_Technical_PO_Item_Units u WHERE u.POItemID = pi.POItemID)
            -
            (SELECT COUNT(*)
             ...
             JOIN TBL_Technical_PO_Item_Units u2 ON u2.POItemUnitID = riu.POItemUnitID
             WHERE u2.POItemID = pi.POItemID
               AND riu.ReturnedFlag = 0
               AND rl.Status IN ('Open','Partial'))
        ELSE
            COALESCE((SELECT SUM(ri3.QtyReceived) FROM TBL_Technical_Receiving_Items ri3 WHERE ri3.POItemID = pi.POItemID), 0)
            - COALESCE((
                SELECT SUM(rli.QtyReleased - COALESCE(rli.QtyReturned, 0))
                FROM TBL_Technical_Release_Items rli
                JOIN TBL_Technical_Release rl2 ON rl2.ReleaseID = rli.ReleaseID
                WHERE rli.POItemID = pi.POItemID AND rl2.Status IN ('Open','Partial')
            ), 0)
        END AS AvailableQty
    FROM TBL_Technical_PO_Items pi
    JOIN TBL_Technical_PO po ON po.POID = pi.POID
    WHERE pi.UnitBarcode IS NOT NULL
) x
WHERE x.AvailableQty > 0
ORDER BY x.ItemDescription ASC, x.ItemCondition ASC";
        ...
        <tr>
          ...
          <td>
            <div class="rowactions">
              ...
              <button class="iconbtn release-print-btn"
                      data-id="<?php echo (int)$rel['ReleaseID']; ?>"
                      title="Print release">
...
<div class="modal-overlay" id="releaseModal">
  <div class="modal modal--wide" role="dialog" aria-modal="true" aria-labelledby="releaseModalTitle">
    <div class="modal__head">
      <div>
        <h3 id="releaseModalTitle">New Release</h3>
    ...
    <form id="releaseForm">
      <div class="modal__body">
          ...
          <div class="field">
            ...
            <select id="relDepartment" required>
              ...
              <?php foreach ($departments as $d):
                $dn = $d['DepartmentName'] ?: ($d['Department'] ?: ($d['DepartmentCode'] ?? ''));
              ?>
          ...
          <div class="field">
            ...
            <select id="relItemName">
              ...
              <option value="<?php echo (int)$s['POItemID']; ?>"
                      ...
                      data-unit-barcode="<?php echo htmlspecialchars($s['UnitBarcode'] ?? ''); ?>"
                      data-description="<?php echo htmlspecialchars($s['ItemDescription'] ?? ''); ?>"
                      data-condition="<?php echo $condition; ?>"
                      data-tracking-method="<?php echo htmlspecialchars($s['TrackingMethod'] ?? 'Quantity-Based'); ?>"
                ...
                (<?php echo $available; ?> <?php echo ($s['TrackingMethod'] ?? '') === 'Individual Unit Tracking' ? 'unit' . ($available !== 1 ? 's' : '') : 'available'; ?>)
              </option>
              <?php endforeach; ?>
            </select>
          </div>
        ...
        <!-- Select Units checklist -- shown only when the identified item uses Individual Unit Tracking -->
        <div id="relUnitsWrap" style="display:none; margin-bottom:16px;">
          <div class="field-group__label" style="margin-bottom:8px;">Select Units <span id="relUnitsCount" class="hint"></span></div>
          <div class="table-wrap" style="max-height:180px; overflow-y:auto;">
            <table class="ledger">
              <tbody id="relUnitsBody"></tbody>
            </table>
          </div>
        </div>
...
<script src="../assets/js/release.js?v=<?php echo @filemtime(__DIR__ . '/../assets/js/release.js') ?: time(); ?>"></script>
