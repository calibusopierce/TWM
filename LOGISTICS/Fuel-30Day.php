...
if ($dateActive) {
    ...
    WITH RA AS (
        SELECT f.PlateNumber, COALESCE(NULLIF(f.Department,''), ts.Department) AS Department,
            COUNT(DISTINCT f.Fueldate) AS DaysRefueled, COUNT(f.FuelID) AS TotalRefuels,
            ROUND(SUM(f.Liters),2) AS TotalLiters, ROUND(AVG(f.Liters),2) AS AvgLiters,
            ROUND(SUM(f.Amount),2) AS TotalAmount, ROUND(AVG(f.Amount),2) AS AvgAmount
        FROM [dbo].[Tbl_fuel] f
    ...
    FROM RA ra LEFT JOIN [dbo].[Vehicle] v ON v.PlateNumber = ra.PlateNumber
...
<title>30-Day Monitor — Fuel Dashboard</title>
